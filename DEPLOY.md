...
4. **Railway автоматически**:
   - Обнаружит `Dockerfile` и использует его для сборки
   - Установит FFmpeg и все системные зависимости
   - Установит Python зависимости из `requirements.txt`
   - Определит GPU (если доступен)
   - Развернёт проект

**Примечание**: Railway автоматически использует Dockerfile, если он присутствует в корне проекта
...
- **Dockerfile**: Используется для установки FFmpeg и всех зависимостей
- **FFmpeg**: Установлен в Dockerfile (требуется для faster-whisper)
