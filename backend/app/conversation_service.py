...
def analyze_with_ai(
    ...
    # Специфичные инструкции для разных уровней
    level_instructions = {
        "A1": """- Исправляй ТОЛЬКО критичные ошибки (мешающие пониманию смысла)
- НЕ придирайся к мелочам (опечатки, артикли в простых фразах)
- Объяснения: максимально просто, 1 предложение, БЕЗ грамматических терминов
- Похвали, если понятно, даже с мелкими ошибками""",
        "A2": """- Исправляй грамматические ошибки и порядок слов
- Можешь указывать на артикли и предлоги
- Объяснения: просто, 1-2 предложения, минимум терминов
- Будь поддерживающим""",
        "B1": """- Исправляй все грамматические ошибки
- Объясняй ПРИЧИНЫ ошибок
- Объяснения: 2-3 предложения, можно использовать термины
- Указывай на стиль и выбор слов""",
        "B2": """- Исправляй ошибки и нюансы
- Указывай на стиль, естественность выражений
- Объяснения: детальные, можно использовать термины
- Улучшай формулировки для более естественного звучания"""
    }
    
    # Определяем язык для ответа бота (на языке диалога)
    response_language_instruction = f"Отвечай на {lang_names.get(language, language)} языке."
    if language == "de":
        response_language_instruction = "Отвечай на немецком языке."
    elif language == "en":
        response_language_instruction = "Отвечай на английском языке."
...
Твоя задача:
1. Определить, что хотел сказать ученик
2. Проверить правильность (грамматика, порядок слов, артикли, времена, предлоги)
3. Если есть ошибки - предложить исправленный вариант
4. Дать краткое объяснение (на русском, простым языком, без сложных терминов на A1-A2)
5. Продолжить диалог естественным образом

{level_instructions.get(level, level_instructions["B1"])}

Важно:
- {response_language_instruction}
- Будь дружелюбным и поддерживающим
- Отвечай коротко (1-2 предложения для диалога)
- Используй emoji умеренно (✅ для правильно, ❌ для ошибок, 💡 для объяснений)
- НЕ "учи неправильно" - если ошибка критична, исправь
- На уровнях A1-A2: если можно понять смысл, похвали

Формат ответа (ОБЯЗАТЕЛЬНО JSON):
{{
    "is_correct": true/false,
    "corrected_text": "исправленный текст (если ошибки) или оригинал (если правильно)",
    "explanation": "краткое объяснение на русском (пустое, если правильно)",
    "bot_response": "твой короткий ответ для продолжения диалога на {lang_names.get(language, language)} языке (1-2 предложения)"
}}"""
